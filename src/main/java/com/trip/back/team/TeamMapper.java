...
public interface TeamMapper {
	...
	@Insert("INSERT INTO notification (account_id, title, keyword, create_at, checked) " + 
			"SELECT f.account_id as account_id,  #{title}, #{keyword}, now(), false "
			+ "FROM follow f JOIN team t ON f.team_id = t.id " + 
			"WHERE t.id = #{teamId}")
	void insertNotification(@Param("teamId")Long teamId, @Param("title") String title, @Param("keyword") String keyword);
	
	// 팀 생성은 db에서만
	@Select("select n.id, account_id accountId, title, keyword, create_at createAt, checked from notification n join accounts a on n.account_id = a.id "
